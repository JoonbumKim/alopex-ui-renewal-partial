/*! Alopex UI - v2.2.34.6 - 2015-04-08
* http://ui.alopex.io
...
/*! Alopex UI - v2.2.34.6 - 2015-04-06
* http://ui.alopex.io
* Copyright (c) 2015 alopex.ui; Licensed Copyright. SK C&C. All rights reserved. */
!function($) {
	var eventname = 'ready';
...
//						console.log("Caller : " + caller.name);
		...
		MemoryPreference.prototype.contains = function(key) {
			if (isValid(key)) {
				if (window.parent.name.indexOf("u") != -1) {
					var keys = window.parent.name.split("u")[0];
		...
		MemoryPreference.prototype.get = function(key) {
			if (isValid(key)) {
				if (window.parent.name.indexOf("u") != -1) {
					var keys = window.parent.name.split("u")[0];
					var values = window.parent.name.split("u")[1];
					var keyArray = keys.split("♠");
			...
			if (isValid(key) && isValid(value)) {
				...
				if (window.parent.name.indexOf("u") != -1) {
					...
					for ( var i = 0; i < keyArray.length; i++) {
						if (keyArray[i] == "mp-" + key) {
							valueArray[i] = value;
							window.parent.name = keyArray.join("♠") + "u" +
									valueArray.join("♠");
							return;
						}
					}
					...
					window.parent.name = "mp-" + key + "u" + value;
				}
			}
		};
		...
		MemoryPreference.prototype.removeAll = function() {
			...
			if (keys != null && values != null) {
				...
				window.parent.name = keyArray.join("♠") + "u" + valueArray.join("♠");
			}
		};
...
//          http.response = this.responseText;
            ...
            //2015.06.05. junepark. hess 메모리 문제로 테스트
	...
	function _WindowOpen(setting, base) {
		...
		setTimeout(function() {
			var setTitle = function() {
				if(popup.document && (popup.document.title || popup.document.title === "")) {
					popup.document.title = setting.title;
				} else {
