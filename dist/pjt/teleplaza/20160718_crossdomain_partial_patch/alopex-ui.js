/*! Alopex UI - v2.3.1.7 - 2016-07-18-01
* http://ui.alopex.io
...
(function($) {
	...
	$.alopex.widget.dateinput = $.alopex.inherit($.alopex.widget.object, {
		...
		init : function(el, options) {
			...
			// 20150731
			// format이 디폴트로 'yyyy-MM-dd'가 있기 때문에 !options.format는 현재 무조건 false이므로 조건문 수정
//			if (options.pickertype == 'monthly' && !options.format) {
			if (options.pickertype == 'monthly') {
		...
		init: function(el, options) {
			...
			if (el.buttons.length > 0) {
				...
				if(this._getProperty(el,'contentsdisplay') == 'flex') {
					buttonWidth = el.buttonGroup.innerWidth() / el.buttons.length;
				}
...
//					}
		...
		addTab: function() {
			...
			if(this._getProperty(tabs,'contentsdisplay') == 'flex') {
				var buttons = tabs.buttonGroup.find('li.af-tabs-button');
				var buttonWidth = tabs.buttonGroup.innerWidth() / (buttons.length + 1);
			...
			this._setTabWidth(tabs);
			this._unbindEvent(tabs);
			this._bindEvent(tabs);
		},
...
(function($) {
	...
	$.alopex.widget.tree = $.alopex.inherit($.alopex.widget.object, {
		...
		_expand : function(li) {
			...
			if(tree.childData) {
				var info = $.alopex.widget.tree._getTreeNodeObj(li);
				
				if(tree.childData[info.id] || tree.childData[info.text] ){
					childUl.empty();
					var jsonArray;
					if(info.id) {
						jsonArray = tree.childData[info.id];
						tree.childData[info.id] = null;
					} else {
						jsonArray = tree.childData[info.text];
						tree.childData[info.text] = null;
					}
					$.alopex.widget.tree._createNode(childUl, jsonArray); // same structure	
					$.alopex.widget.tree.refresh(parentUl, jsonArray);
				}
			}

			$(li).attr('data-expand', 'true').addClass('af-tree-expanded Expanded');
			childUl.css('display', 'block');
			$.alopex.widget.tree._triggerEvent(parentUl, li, 'expand');
		},
		...
		setDataSource : function(el, data, dynamic) {
			$(el).empty();
			if(dynamic) {
				var parsedData = $.alopex.widget.tree._parse(data);
				el.childData = parsedData[1];
				$.alopex.widget.tree._createNode(el, parsedData[0]); // same structure
			} else {
		...
		_parse : function(data) {
			...
			function parse(data) {
				if(data.id) {
					result[1][data.id] = data.items;
				} else {
					result[1][data.text] = data.items;
				}
		...
		_structure : function(ul) {
			...
			$(ul).find('> li')./*addClass('af-tree-node').*/each(function() {
				...
				if (isExpandable) {
					...
					if ($span.length === 0) {
						$('<span></span>').attr('class', 'af-tree-icon Arrow').attr('data-expand', 'false').insertBefore($(this).find('> a')[0]);
					}
		...
		_addCheckbox : function(el) {
			...
			if ($(el).closest('.Tree').attr('data-checkbox') === 'visible') {
				$(el).find('input[type="checkbox"]').css('display', 'inline-block');
			}
		},
		...
		_checkCheckbox: function(el, data) {
			 if(data instanceof Array) {
				for( var i = 0; i < data.length; i++) {
					this._checkCheckbox(el, data[i]);
				}
			} else if(typeof data === 'object') {
				$(el).find('li').each(function() {
					if(data.id) {
						if(this.id === data.id) {
							check($(this).find('.af-checkbox'), data)
						}
					} else {
						$(this).find('.af-tree-link').each(function() {
							if($(this).text === data.text) {
								check($(this).parent().find('.af-checkbox'), data)
							}
						});
					}
				});

				function check(list, data) {
					list.each(function() {
				...
				if(data.items && (data.items instanceof Array) && (data.items.length > 0)) {
					this._checkCheckbox(el, data.items);
				}
			}
		}
	});
