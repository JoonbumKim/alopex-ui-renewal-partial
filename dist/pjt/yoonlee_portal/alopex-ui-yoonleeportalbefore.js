/*! Alopex UI - v2.3.1.14 - 2016-01-14
* http://ui.alopex.io
...
(function($) {
	...
	$.alopex.keyfilter = {
		...
		addRule: function(name, allowable) { // 신규 추가된 대안
			if(typeof arguments[0] === 'string' && arguments[0] !== '') {
				if(typeof arguments[1] === 'string' && arguments[1] !== ''){
					builtInRegexp[name] = allowable; // keyup에서 처리함
				}
			}
		}
	};
...
//		    });
		...
		_setDialogType: function(el, type) {
		  ...
		  if (type === 'blank' || type === 'close' || type === 'confirm' || type === 'okcancel') {
		    ...
		    /**
		     * [Hong-HyunMin 2016.01.05 TODO] 기존에 동작하지 않았으나 2016.01.05 요청사항에 의해 동작하도록 변경.
		     * DialogToggle class는 현재 theme에만 적용되어 있음. 해당 기능을 http://ui.alopex.io/ 기준의 alopex-ui.css에서도 사용 하도록 확인 필요.
		     */
		    if(el.toggle) {
		      ...
		      $(togglebtn).click(function(e) {
		    	...
		    	//펼칠때의 기준이 되는 높이
		    	var standardHeight = $.alopex.util.isValid(el.height)==true?el.height:el.originHeight;
		    	//접을대의 기준이 되는 높이.
		    	var headerHeight = $.alopex.util.isValid($(e.currentTarget.dialog).find(".Header").height())==true?$(e.currentTarget.dialog).find(".Header").height():0;  
		    	...
		    	if(e.currentTarget.dialog.folded) {
		          ...
		          $(e.currentTarget.dialog).animate({height: standardHeight +'px'}, 300, function() {
	        	     if (el.scroll || (el.contents && el.contents.attr("data-dialog-scroll")) === "true") {
			    		  ...
			    		  //[Hong-HyunMin 2016.01.05] 스크롤이 존재하는 경우, Dialog Height에 스크롤 영역이 가려서 보이지 않는 경우에 스크롤이 보이도록 처리.
			    		  var optionScroll = $.alopex.util.isValid(el.scroll)==true?el.scroll:false;		  
			    		  var markUpScroll = $.alopex.util.isValid($(parent).find(".contents").attr('data-dialog-scroll'))==true?$(parent).find(".contents").attr('data-dialog-scroll'):false;
			    		  
			    		  if(true == optionScroll || true == markUpScroll){
			    			  $el.find('[data-dialog-scroll=true]').css('height', (parentHeight - siblingHeight - 1) + 'px');
			    		  }
			    		  else {
		...
		_resizeLeftDialog: function(e, ae) {
		   ...
		   */
		  var standardWidth = $.alopex.util.isValid(parent.width)==true?parent.width:parent.originWidth;
		...
		_resizeBottomDialog: function(e, ae) {
		   ...
		   * optionScroll    : dialog.open() 시에 적용한 scroll option
		   * markUpScroll    : dialog의 contents 영역에 설정한 data-dialog-scroll markup attr
		   * optionResizable : dialog.open() 시에 적용한 resize option
		   * markUpResizable : dialog의 contents 영역에 설정한 data-resizable markup attr
		   */
		  var optionScroll = $.alopex.util.isValid(parent.scroll)==true?parent.scroll:false;		  
		  var markUpScroll = $.alopex.util.isValid($(parent).find(".contents").attr('data-dialog-scroll'))==true?$(parent).find(".contents").attr('data-dialog-scroll'):false;
		  var optionResizable = $.alopex.util.isValid(parent.resizable)==true?parent.resizable:false;
		  var markUpResizable = $.alopex.util.isValid($(parent).find(".contents").attr('data-resizable'))==true?$(parent).find(".contents").attr('data-resizable'):false;
		  /**
		   * [Hong-HyunMin 2016.01.05] 옵션이 스크롤이거나 마크업에 리사이즈 속성을 적용한 경우.
		   */
		  if(true == optionScroll || true == markUpScroll){
			  /**
			   * [Hong-HyunMin 2016.01.05] 옵션이 리사이즈이거나 마크업에 리사이즈 속성을 적용한 경우.
			   */
			  if(true == optionResizable || true == markUpResizable){
				  
				  //Dialog dialog.open() 시에 적용한 title option에 의한 Header영역
				  var headerHeight = $.alopex.util.isValid($(parent).find(".Header").height())==true?$(parent).find(".Header").height():0;
				  
				  //Dialog의 Height가 option에 설정한 Height까지 resize 되는 것이 아니라, originHeight resize되기 때문에
				  //option에 Height를 지정한 경우, 해당 값으로 처리하고 그렇지 않은 경우에만 originHeight를 사용하도록 변경함.
				  var standardHeight = $.alopex.util.isValid(parent.height)==true?parent.height:parent.originHeight;
				  ...
				  if (resizedY < standardHeight) {
					  ...
					  $(parent).find(".contents").css('height', standardHeight - headerHeight - 1 + 'px');
				  }
				  else{
					  $(parent).css('height', resizedY + 'px');
					  $(parent).find(".contents").css('height', resizedY - headerHeight - 1 + 'px');
				  }
		  ...
		  //[Hong-HyunMin 2016.01.05]스크롤 옵션이 없는 경우의 리사이즈는 기존 방식으로 처리하여 영향도가 없도록 함.
		  else{
...
(function($) {
	$.alopex.widget.tabs = $.alopex.inherit($.alopex.widget.object, {
		...
		setters: ['tabs', 'setTabIndex','setTabByContent', 'setEnabled', 'setEnabledByContent', 'reload', 'clear', 'removeTab', 'setBeforeunload', 'addTab'],
...
//				}
		...
		function MemoryPreference() {
			...
			// IE 버그 20160106 ESH
			// 크로스도메인 화면1 > window.open(url, '특정문자열') > Alopex도메인 화면2 > $a.navigate > Alopex도메인 화면2 으로 실행하면
			// 화면2의 window.parent.name 값이 화면2에서 저장한 데이터($a.parameters, $a.session 등)가 아닌 그 전에 없어진 '특정문자열'이 다시 return되면서
			// memorypreference.get 할 때 $a.parameters, $a.session 등을 get 하지 못함
			// IE는 sessionStorage를 이용해 $a.parameters, $a.session 등을 저장한다
...
//			if(!!(navigator.userAgent.match(/Trident/) && navigator.userAgent.match(/rv[ :]11/))) {
				...
				if(checkAlopexWindowParent()) this.data = window.parent.name;
			}
		}
...
//			if(!!(navigator.userAgent.match(/Trident/) && navigator.userAgent.match(/rv[ :]11/))) {
				...
				if(checkAlopexWindowParent()) window.parent.name = data;
			}	
		}
...
//			$dialog.find('iframe')[0].contentWindow.name = setting.id;
	...
	function _WindowOpen(setting, base) {
		...
		if (setting.modal === true) {
			...
			if(checkAlopexWindowParent()) $a.block(window.parent);
		}
...
//            }
						// AFTER !!!!!!!!
            			// 순서 platform(http.request success콜백 일때는 무조건 호출) > 공통(또는 코어 디폴트, isSuccess === true 일때만) > 사용자(isSuccess === true 일때만)
						...
						// FAIL !!!!!!!! (isSuccess === false 일때만 호출)
            			// 순서 platform > 공통 > 사용자(isSuccess === true 일때만)
...
/*! Alopex UI - v2.3.1.14 - 2016-01-13
* http://ui.alopex.io
* Copyright (c) 2016 alopex.ui; Licensed Copyright. SK C&C. All rights reserved. */
!function($) {
