/*! Alopex UI - v2.2.34.5 - 2015-03-27
* http://ui.alopex.io
...
(function($) {
	...
	$.alopex = $.extend($.alopex, {
		...
		config: {
			...
			selectedClassName: 'af-selected',
			disabledClassName: 'af-disabled',
		...
		convert: function(root) {
			...
			if($.alopex.config.classLoader) {
				for(var i in $.alopex.config.defaultComponentClass) {
					$(root).find('.'+ $.alopex.config.defaultComponentClass[i]).each(function(idx, element) {
						$(element).attr({
							'data-type': i,
							'data-classinit': 'true'
						});
				...
				for(var i in $.alopex.config.componentSelector) { /* 컴퍼넌트화 할 셀렉터  */
					$(root).find(i).each(function(idx, element) {
...
(function($) {
	...
	$.alopex.widget.dateinput = $.alopex.inherit($.alopex.widget.object, {
		...
		init : function(el, options) {
			...
			if(isClassLoad) {
				var $wrapper = $(wrapper);
				if($wrapper.find('.calendar').length>0) {
					el.imagebutton = $wrapper.find('.calendar');
				}
				if($wrapper.find('.clear').length>0) {
					el.resetbutton = $wrapper.find('.clear');
				}
			} else {
	...
	$.alopex.widget.daterange = $.alopex.inherit($.alopex.widget.object, {
	  ...
	  init: function(el, options) {
	    ...
	    // var $inputs = $el.find('input');
		  ...
		  el._start = $el.find('[data-role="' + this.id_startdate + '"], .startdate');
		  el._end = $el.find('[data-role="' + this.id_enddate + '"], .enddate');
...
(function($) {
	...
	$.alopex.widget.dropdown = $.alopex.inherit($.alopex.widget.object, {
		...
		open: function(el) {
		  ...
		  var renderWidth = $el.offset().left + dropdownWidth;
		  var windowWidth = $(window).width();
		  if (renderWidth > windowWidth) {
		    left = left - (renderWidth - windowWidth + el.margin);
		  }
		  var renderHeight = $el.offset().top + dropdownHeight;
				...
				if($(input).parent('label').length>0) {
					...
					for(var i=0; i<$inputs.length; i++) { // radio가 들어가 있는 경우 고려하여 loop 돌면서 체크 
						if($inputs.eq(i).is(':checked')) {
							$inputs.eq(i).parent('label').addClass('af-checked');
						} else {
							$inputs.eq(i).parent('label').removeClass('af-checked');
						}
					}
				}
...
//			console.log($el, $el.find('.selected').attr('data-page'));
...
//				targetpage = 1;
		...
		_next: function(e) {
			console.log('_next == ', e.currentTarget);
			var el = e.currentTarget.element;
			if (el.selectedPage !== el.totalpage) {
				var targetpage = el.selectedPage + 1;
				$.alopex.widget.pagination.setSelectedPage(el, targetpage);
...
(function($) {
	...
	$.alopex.widget.spinner = $.alopex.inherit($.alopex.widget.baseinput, {
		...
		init: function(el, options) {
			...
			$el = $(el);
			el.type = 'text'; // number로 지정할 경우, 화살표가 자동으로 생겨 text로 재설정.
...
//			}
		...
		refreshButtons: function(el, index, setFocus) {
			...
			if (setFocus == true) {
				...
				el.buttonGroup.children().eq(index).attr('aria-selected', 'true').attr('tabindex', '0').addClass('af-selected');
			}
		},
...
(function($) {
	...
	$.alopex.widget.tree = $.alopex.inherit($.alopex.widget.object, {
		...
		_removeEvent : function(el) {
			$(el).find('.af-tree-icon, span.icon').unbind('click');
		...
		_addBasicEvent : function(el) {
			$(el).find('.af-tree-icon, span.icon').bind('click', function(e) {
