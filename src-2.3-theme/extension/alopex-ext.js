...
(function($, undefined) {
  ...
  $.widget("ech.multiselect", {
    ...
    _setMenuWidth: function() {
      ...
      this.options.menuWidth = this.element.outerWidth() === 'auto' ? this.element.outerWidth() : this.options.menuWidth; 
    ...
    _setOption: function(key, value) {
      ...
      switch(key) {
      	case 'checkedheader':
          menu.find('.ui-multiselect-all')[value ? 'show' : 'hide']();
          menu.find('.ui-multiselect-none')[value ? 'show' : 'hide']();
          break;
        case 'header':
...
// jQuery Upload File Plugin에서 $.fn.ajaxForm == undefined 일 경우,
// <script async src="http://malsup.github.io/jquery.form.js?_=1469584282678"></script> 이 테그를 통해 동적 lib 호출 하는데
// github 도메인 등이 프로젝트에서 막혀 있기에
// ext.js 에 추가 하거나, jquery.form.min.js를 페이지에 include 해줘야 함
/*!
 * jQuery Form Plugin
...
(function ($) {
    ...
    $.fn.uploadFile = function (options) {
        ...
        //Alopex
        var chkCtrlBtn = '<button type="button" id="btn_setCheckedAllTrue_'+s.showQueueDiv+'" class="Button">'+s.checkAllStr+'</button>';
		chkCtrlBtn +='<button type="button" id="btn_setCheckedAllFalse_'+s.showQueueDiv+'" class="Button">'+s.unCheckAllStr+'</button>';
		chkCtrlBtn +='<button type="button" id="btn_cancelChecked_'+s.showQueueDiv+'" class="Button">'+s.checkedDeleteStr+'</button>'
        ...
        (function checkAjaxFormLoaded() {
            if($.fn.ajaxForm) {
                ...
                if(s.dragDrop) {
                	if(s.selecttype == "advance"){
                		$(obj).append(chkCtrlBtn);
                		setChkHandlers(obj);
                	}
...
//                    $(dragDrop).append(uploadLabel);
               ...
               // $(obj).append(obj.errorLog);Alopex

   				if(s.showQueueDiv)
		        	obj.container =$("#"+s.showQueueDiv);
        		else
		            obj.container = $("<div class='Container'></div>").insertAfter($(obj));
	   ...
	   this.startUpload = function () {
        ...
        this.update = function (settings) {
            //update new settings
            s = $.extend(s, settings);
          //Alopex - 사용자 onLoad 이벤트 설정 시 바로 반영하는 로직
            if(settings.onLoad != undefined){
            	s.onLoad.call(settings.onLoad, obj);
            }
        }
        ...
        this.createProgress = function (filename,filepath,filesize) {
            ...
            if(s.showPreview)
            {
            	//Alopex - 이미 서버에 올라간 파일 정보를 파일업로드 컴포넌트 로드시 표시할 때 프리뷰 썸네일 체크
            	var thumbext = filepath; 
            ...
            if(s.showDownload) {
                ...
                pd.download.click(function () {
                	//Alopex - 파일명이 배열 형식으로 들어가던 것을 skip
            	    //이미 업로드된 파일의 다운로드와 파일추가후 업로드 후 다운로드와 다른게 적용하여서 
            	    //일관된 파일명 접근을 위해 통일.
            ...
            if(s.showDelete)
            {
    	        ...
    	        pd.del.click(function () {
            	    ...
            	    var arr = [filename];
            	    //Alopex - 파일명이 배열 형식으로 들어가던 것을 skip
            	    //이미 업로드된 파일의 삭제와 파일추가후 업로드 후 삭제와 다른게 적용하여서 
            	    //일관된 파일명 접근을 위해 통일.
                	if(s.deleteCallback) s.deleteCallback.call(this, filename, pd);
	                obj.selectedFiles -= 1;
        ...
        //Alopex
          function setChkHandlers(obj) {
        	  
        	  $('#btn_setCheckedAllTrue_'+s.showQueueDiv).on('click', function(){
        			var $obj_listBox = $(obj).next();
            		$obj_listBox.find('.Checkbox').setCheckAll(true);
        	  });
        	   
        	  $('#btn_setCheckedAllFalse_'+s.showQueueDiv).on('click', function(){
        			var $obj_listBox = $(obj).next();
            		$obj_listBox.find('.Checkbox').setCheckAll(false);
        	  });
              
        	  $('#btn_cancelChecked_'+s.showQueueDiv).on('click', function(){
        		  var $el_listBox = $(obj).next();
        			var chks=$el_listBox.find("input:checked");
        			for( var i = 0 ; i < chks.length ; i++ ){
        				var pb = $(chks[i]).closest('.preview-list');
        				$(pb).find('.cancel').click();
        				$(pb).find('.del').click();
        			}
      	      });
          }
            ...
            ddObj.on('drop', function (e) {
                ...
                if(!s.multiple && files.length > 1) {
                  ...
                  //  if(s.showError) $("<div class='" + s.errorClass + "'>" + s.multiDragErrorStr + "</div>").appendTo(obj.errorLog);
                	  if(s.showError) s.onError.call(this, files, "Alert", s.multiDragErrorStr, "");
                	return;
                }
		    ...
		    var ts = s;
                ...
                if (!isFileTypeAllowed(obj, s, files[i].name)) {
                   // if (s.showError) $("<div><font color='red'><b>" + files[i].name + "</b> " + s.extErrorStr + s.allowedTypes + "</font></div>").appendTo(obj.errorLog);
                	...
                	//Alopex
                    if(s.showError) s.onError.call(this, files[i].name, "Alert", s.extErrorStr+s.allowedTypes, "");
                ...
                if (s.maxFileSize != -1 && files[i].size > s.maxFileSize) {
                   // if (s.showError) $("<div><font color='red'><b>" + files[i].name + "</b> " + s.sizeErrorStr + getSizeStr(s.maxFileSize) + "</font></div>").appendTo(obj.errorLog);
                    //Alopex 
                    if(s.showError) s.onError.call(this, files[i].name, "Alert", s.sizeErrorStr + getSizeStr(s.maxFileSize), "");
        ...
        function serializeAndUploadFiles(s, obj, files) {
            for(var i = 0; i < files.length; i++) {
                ...
                if(!s.allowDuplicates && isFileDuplicate(obj, files[i].name)) {
                   ...
                   //Alopex
                    if(s.showError) s.onError.call(this, files[i].name, "Alert", s.duplicateErrorStr,"");
                    continue;
                }
                if(s.maxFileSize != -1 && files[i].size > s.maxFileSize) {
...
//                        obj.errorLog);
                  ...
                  //Alopex
                    if(s.showError) s.onError.call(this, files[i].name, "Alert", s.maxFileCountErrorStr + s.maxFileCount,"");
        ...
        function createCustomInputFile (obj, group, s, uploadLabel) {
            ...
            fileInput.change(function () {
            	//Alopex 추가
            	if(s.selecttype == "basic"){
            		$("form").each(function (i, items) {
                        if($(this).hasClass(obj.formGroup)) $(this).remove();
                    });
           		 obj.reset();	
            	}
...
//                            obj.errorLog);
                      ...
                      //Alopex 
                        if(s.showError) s.onError.call(this, filenameStr, "Alert", s.extErrorStr + s.allowedTypes, "");
...
//                            obj.errorLog);
                      ...
                      //Alopex 
                        if(s.showError) s.onError.call(this, fileList, "Alert", s.maxFileCountErrorStr + s.maxFileCount, "");
                ...
                success: function (data, message, xhr) {
                    ...
                    if(s.returnType == "json" && $.type(data) == "object" && data.hasOwnProperty(s.customErrorKeyStr)) {
                        ...
                        if(s.showStatusAfterError) {
                            pd.progressDiv.hide();
                            $(pd.statusbar).addClass("upload-error") //Alopex
                         //   pd.statusbar.append("<span class='" + s.errorClass + "'>ERROR: " + msg + "</span>");//Alopex
                        } else {
                            pd.statusbar.hide();
                            pd.statusbar.remove();
                        }
                ...
                error: function (xhr, status, errMsg) {
                    ...
                    if(xhr.statusText == "abort") //we aborted it
                    {
                        ...
                        if(s.showStatusAfterError) {
                            ...
                            $(pd.statusbar).addClass("upload-error") //Alopex
                          //  pd.statusbar.append("<span class='" + s.errorClass + "'>ERROR: " + errMsg + "</span>"); //Alopex
                        } else {
                            pd.statusbar.hide();
                            pd.statusbar.remove();
                        }
